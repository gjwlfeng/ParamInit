...
public class ActivityParamStringHolder extends ActivityParamArrayHolder {

    private ActivityParamStringHolder(AnnotationEnv annotationEnv, Element element, boolean isSupportV4, boolean isAndroidX) {
        ...
        if (paramInit.inject()) {
            ...
            MethodSpecUtils.codeBlock(methodSpec, new MethodSpecBuilderCallBack() {
                ...
                public boolean innerBlock(MethodSpec.Builder builder) {
                    builder.addStatement("activity.$N =$N.getString($L)",
                            getOriginFiledName(),
                            globalVarName,
                            getParamFiledName());
                    return false;
    ...
    public void onInitMethodWithIntent(MethodSpec.Builder methodSpec, String globalVarName) {
        ...
        ParamInitActivity paramInit = getAnnotation(ParamInitActivity.class);
        if (paramInit.inject()) {
            methodSpec.beginControlFlow("if ($N.hasExtra($N))",globalVarName, getParamFiledName());
            MethodSpecUtils.codeBlock(methodSpec, new MethodSpecBuilderCallBack() {
                @Override
                public boolean innerBlock(MethodSpec.Builder builder) {
                    TypeMirror typeMirror = element.asType();
                    builder.addStatement("activity.$N = $N.getStringExtra($N)",
                            getOriginFiledName(),
                            globalVarName,
                            getParamFiledName());
                    return false;
    ...
    public void onSaveStateMethod(MethodSpec.Builder methodSpec) {
        ...
        if (paramInit.persistence()) {
            ...
            MethodSpecUtils.codeBlock(methodSpec, new MethodSpecBuilderCallBack() {
                @Override
                public boolean innerBlock(MethodSpec.Builder builder) {
                    builder.addStatement("savedInstanceState.putString($L,activity.$N)",
                            getParamFiledName(),
                            getOriginFiledName());
                    return false;
                }
    ...
    public static class CreationHolder extends ActivityCreationHolder<ActivityParamStringHolder> {

        public CreationHolder(AnnotationEnv annotationEnv, Element element, boolean isSupportV4, boolean isAndroidX) {
            super(annotationEnv, element, isSupportV4, isAndroidX);
        }

        public ActivityParamStringHolder getHolder() {
            return new ActivityParamStringHolder(this.annotationEnv, this.element, isSupportV4, isAndroidX);
        }
    }
}
