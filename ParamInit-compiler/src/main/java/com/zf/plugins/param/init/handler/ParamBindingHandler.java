...
public abstract class ParamBindingHandler<T> extends BaseHandler<T> {

    public ParamBindingHandler(AnnotationEnv annotationEnv, TypeElement rootElement, List<? extends Element> itemElement, List<? extends Element> paramBindingElement, boolean isSupportV4, boolean isAndroidX) {
    ...
    protected abstract TypeSpec.Builder createParamBindingClassType();

    protected abstract FieldSpec.Builder onField();

    protected abstract MethodSpec.Builder onConstructorMethod();

    protected abstract List<MethodSpec.Builder> onBindingMethod();

    protected abstract List<MethodSpec.Builder> onSetMethods(List<T> holders);

    protected abstract List<MethodSpec.Builder> onGetMethods(List<T> holders);

    protected abstract MethodSpec.Builder onGetResult(List<T> holders);

    public TypeSpec.Builder createClassInternal(List<T> holders) {
        TypeSpec.Builder paramBindingClass = createParamBindingClassType();

        FieldSpec.Builder intentFieldBuilder = onField();
        paramBindingClass.addField(intentFieldBuilder.build());

        MethodSpec.Builder constructorMethod = onConstructorMethod();
        paramBindingClass.addMethod(constructorMethod.build());

        List<MethodSpec.Builder> bindingMethods = onBindingMethod();
        for (MethodSpec.Builder bindingMethod : bindingMethods) {
            paramBindingClass.addMethod(bindingMethod.build());
        }

        List<MethodSpec.Builder> ongSetMethods = onSetMethods(holders);
        for (MethodSpec.Builder methodBuilder : ongSetMethods) {
            paramBindingClass.addMethod(methodBuilder.build());
        }

        List<MethodSpec.Builder> ongGetMethods = onGetMethods(holders);
        for (MethodSpec.Builder methodBuilder : ongGetMethods) {
            paramBindingClass.addMethod(methodBuilder.build());
        }

        MethodSpec.Builder ongGetResultMethod = onGetResult(holders);
        paramBindingClass.addMethod(ongGetResultMethod.build());

        return paramBindingClass;
    }
